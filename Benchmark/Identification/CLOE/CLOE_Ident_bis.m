function [Theta, fval, it, flag, Thetas] = CLOE_Ident_bis(Theta_0, data_struct, t_0, t_f, N, optionsCLOE)
...
%     options = optimoptions('fmincon','Display','iter', 'algorithm', 'sqp');
    ...
    Theta_it  = Theta_0;
%     Theta_next_it = zeros(size(Theta_it));
    Theta_next_it =  Theta_0;
    
    it = 1;
    max_it=20;
    
    F = norm(RRR_CLOE_Q(Theta_0, data_struct, t_0, t_f, N, optionsCLOE));
    stop_1 = 1;
    stop_2 = 1;
    S1 = 1e-4;
    S2 = 1e-3;
...
%     figure
    ...
    %         Theta_next_it(i) = fmincon(@(Khi) norm(RRR_CLOE_Q_1_1(Khi,i,Theta_it, data_struct, t_0, t_f, N, optionsCLOE)), Theta_it(i), [], [], [], [], [], [], [], options)
            ...
            Theta_next_it(i) = fminsearch(@(Khi) norm(RRR_CLOE_Q_1_1(Khi,i,Theta_next_it, data_struct, t_0, t_f, N, optionsCLOE)), Theta_next_it(i), options);
% 
%             V = linspace(Theta_next_it(i)-alpha, Theta_next_it(i)+alpha, n);
%             error = zeros(n,1);
%             parfor j = 1:n
%                 Theta_bis = Theta_next_it;
%                 Theta_bis(i)=V(j);
...
%             drawnow
        ...
        stop_2 = abs(norm(RRR_CLOE_Q(Theta_next_it, data_struct, t_0, t_f, N, optionsCLOE))-F);
        
        F = norm(RRR_CLOE_Q(Theta_next_it, data_struct, t_0, t_f, N, optionsCLOE));
    ...
    fval = norm(RRR_CLOE_Q(Theta, data_struct, t_0, t_f, N, optionsCLOE));
