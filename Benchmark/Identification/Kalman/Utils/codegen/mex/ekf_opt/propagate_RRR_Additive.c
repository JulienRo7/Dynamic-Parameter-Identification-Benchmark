...
static emlrtRSInfo j_emlrtRSI = { 6,   /* lineNo */
...
static emlrtRSInfo fb_emlrtRSI = { 29, /* lineNo */
  "RRR_dyn_CL",                        /* fcnName */
  "/home/quentin/Desktop/MatlabCommented05092018/Benchmark/Identification/Utilities/RRR_dyn_CL.m"/* pathName */
};
                ...
                Theta_size[1], const real_T y[6], real_T varargout_1[6])
{
  ...
  if (u_km1_size[0] != 3) {
    emlrtSizeEqCheck1DR2012b(u_km1_size[0], 3, &e_emlrtECI, &st);
  }
  ...
  int32_T q_km1_size[1], real_T x_data[], int32_T x_size[1])
{
  ...
  cell_wrap_1 t0_tunableEnvironment[2];
  ...
  Theta_size_idx_0 = (y - i2) + 1;
  loop_ub = (y - i2) + 1;
  for (y = 0; y < loop_ub; y++) {
    Theta_data[y] = x_km1_data[(i2 + y) - 1];
  }
  ...
  for (i2 = 0; i2 < loop_ub; i2++) {
    tunableEnvironment[0].f1.data[i2] = u_km1_data[i2];
  }

  tunableEnvironment[1].f1.size[0] = Theta_size_idx_0;
  for (i2 = 0; i2 < Theta_size_idx_0; i2++) {
    tunableEnvironment[1].f1.data[i2] = Theta_data[i2];
  }

  for (i2 = 0; i2 < 2; i2++) {
    t0_tunableEnvironment[i2] = tunableEnvironment[i2];
  }
  ...
  emxInit_real_T(sp, &unusedU0, 1, &emlrtRTEI, true);
  ...
  y = b_y->size[0];
  if (!((y >= 1) && (y <= i2))) {
    emlrtDynamicBoundsCheckR2012b(y, 1, i2, &emlrtBCI, sp);
  }

  emxFree_real_T(sp, &unusedU0);
  for (i2 = 0; i2 < 6; i2++) {
    tmp_data[i2] = b_y->data[(y + b_y->size[0] * i2) - 1];
  }
  ...
  i2 = 6 + Theta_size_idx_0;
  if (i2 != q_km1_size[0]) {
    emlrtSizeEqCheck1DR2012b(i2, q_km1_size[0], &d_emlrtECI, sp);
  }

  x_size[0] = 6 + Theta_size_idx_0;
  loop_ub = 6 + Theta_size_idx_0;
  for (i2 = 0; i2 < loop_ub; i2++) {
    x_data[i2] = tmp_data[i2] + q_km1_data[i2];
  }
